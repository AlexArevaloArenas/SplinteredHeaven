        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity))
        {
            if(hit.collider.gameObject.layer == LayerMask.NameToLayer("Terrain"))
            {
                foreach (GameObject unit in UnitSelections.Instance.unitSelected)
                {
                    if (unit.tag == "Player")
                    {
                        unit.GetComponent<CharacterManager>()?.StartMoveBehaviour(hit.point);
            ...
            if(hit.collider.gameObject.layer == LayerMask.NameToLayer("Selectable"))
            {
                ...
                foreach (GameObject unit in UnitSelections.Instance.unitSelected)
                {
                    if (unit.tag == "Player")
                    {
                        unit.GetComponent<CharacterManager>()?.StartAttackBehaviour(hit.collider.GetComponent<UnitManager>());
