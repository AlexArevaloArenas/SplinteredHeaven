...
public class FOVManager : MonoBehaviour
{
    ...
    //Memory texture for the fog of war, used to store the state of the fog of war between frames
	private RenderTexture FOWMemoryTexture;
	...
	//MEMORY MATERIALS
	[SerializeField]
	private Material memoryBlendMaterial; // Assign a blend shader that keeps explored areas
	...
	private void Start()
	{
        ...
        // Create a memory texture to store the state of the fog of war between frames
        FOWMemoryTexture = new RenderTexture(FOWTextureSize, FOWTextureSize, 0, RenderTextureFormat.ARGB32);
        FOWMemoryTexture.Create();
        FOWMaterial.SetTexture("_MemoryTex", FOWMemoryTexture); // optional, if the shader samples it
	...
	private IEnumerator UpdateFOV()
	{
		...
		void ApplyFOWPass()
		{
			...
			// Blend FOWRenderTexture into memory texture
			Graphics.Blit(FOWRenderTexture, FOWMemoryTexture, memoryBlendMaterial);
